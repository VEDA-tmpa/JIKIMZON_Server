...
namespace storage
{
	...
	// constexpr uint64_t MAX_FILE_SIZE = 1 * 1024 * 1024; // 1MB
	...
	*/
	...
	constexpr uint32_t MAX_DATA_OFFSET = 100 * 1024 * 1024 - sizeof(ItemHeaderStruct);
	...
	void StorageFile<ITEM>::UpdateFileHeader()
	{
		...
		logger.Debug("UpdateFileHeader() mFileHeaderStruct.FirstItemOffset: " + std::to_string(mFileHeaderStruct.FirstItemOffset));
		logger.Debug("UpdateFileHeader() mFileHeaderStruct.LastItemOffset: " + std::to_string(mFileHeaderStruct.LastItemOffset));
		logger.Debug("UpdateFileHeader() mFileHeaderStruct.PaddingOffset: " + std::to_string(mFileHeaderStruct.PaddingOffset));
		file.write(reinterpret_cast<const char*>(&mFileHeaderStruct), sizeof(FileHeaderStruct));
		if (file.fail())
		{
			throw std::runtime_error("Failed to write file header.");
		}

		logger.Debug("UpdateFileHeader() cursor tellp()  ; After file write:  " + std::to_string(file.tellp()));
